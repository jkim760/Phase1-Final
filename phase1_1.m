...
simulT = 100; %(Time Unit:  miliseconds)
dT = .01; %Time Interval 
t= 0:dT:simulT; %Starting from 0 to 100 at timestep of dT


%Injected Current ( affects the change in voltage) - No External Current (Steady
%State: Resting Membrane Potential)
I = zeros(1, length(t)) %Here, both time and current are inputs. They are same in length. From 1 to 100
% the corresponding values of the current input for each timestep is set to zero.
